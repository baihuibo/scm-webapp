...
var newsclientId = '';
var dataresultObj = '';
var gapReceipdataresult = '';
var gapReceiptList=[]    //回传差额收据信息
$(function() {
	$("select").chosen({
	...
	if(contractid){
		...
		jsonGetAjax(
			...
			function(result){
				...
				gapReceipdataresult = result.data.gapReceiptList;
...
function recptList(dataresultdata){
	if(dataresultdata){
			...
			columns:[
		        {field: 'flyid',title :'序号',checkbox:true, align: 'center',
	      	    ...
	      	    {field: 'refundAmount', title: '退款金额', align: 'center',
	      	    	formatter: function(value, row, index){	
	           			var html='';
	           			var editcheckType = 'readonly';
	           			if(row.refundAmount > 0){
	           				editcheckType='';
	           			}
	           			html='<input type="text" id="zhuan'+index+'" class="form-control input_return zhuankuan" '+editcheckType+' name="" value="'+row.refundAmount+'" placeholder="0.00">';
	           			return html;
	           		}
	      	    },
		      	...
		      	{field: 'recycleCount', title: '收据回收张数', align: 'center',
		      		formatter: function(value, row, index){
	      				...
	      				var editcheckType = 'readonly';
	           			...
	           			html='<input type="text" id="'+index+'" class="form-control input_return" name="" '+editcheckType+' value="'+row.recycleCount+'">';
	           			return html;
	           		}
		      	},
		      	{field: 'differentReason', title: '回收差异原因', align: 'center',
		      		formatter: function(value, row, index){
		      			...
		      			if(differentReason !=undefined){
		      				var html='';
		           			html='<input type="text" id="yin'+index+'" class="form-control input_val" name="" '+editcheckType+' value="'+differentReason+'">';
		           			return html;
		      			}else{
		      				var html='';
		           			html='<input type="text" id="yin'+index+'" class="form-control input_val" name="" '+editcheckType+' value="-">';
		           			return html;
		      			}
	           		}
		      	}
		...
		//选中行转款金额计算
		var table = $('#J_refunds_dataTable');
		table.off().on('keyup blur focus' , 'input' ,countTotal );
		function countTotal(){
			var total = 0;
			table.find('tbody tr').filter(function(){
				return $(this).find(':checkbox')[0].checked;
			}).each(function(){
				total += parseFloat($(this).find('.zhuankuan').val());
			});
			$('#J_paymentmoeny').text(total);
		}
		countTotal();
	}
};
...
function gapReceiptListApply(gapReceipdataresult){
	var gapReceiphtml = '';
	gapReceiphtml='\
		<table id="J_gapReceiptList_dataTable" class="table table-hover table-striped table-bordered">\
		<thead>\
			<tr>\
				<th data-field="">\
					<div class="th-inner">新收据编号</div>\
				</th>\
				<th data-field="">\
					<div class="th-inner">生成时间</div>\
				</th>\
				<th data-field="">\
					<div class="th-inner">收款批次号</div>\
				</th>\
				<th data-field="">\
					<div class="th-inner">原收据编号</div>\
				</th>\
				<th data-field="">\
					<div class="th-inner">发票编号</div>\
				</div>\
				</th>\
				<th data-field="">\
				...
				<th data-field="">\
					<div class="th-inner">开票状态</div>\
				</th>\
				<th data-field="">\
					<div class="th-inner">打印状态</div>\
				</th>\
				<th data-field="">\
					<div class="th-inner">是否回收</div>\
				</th>\
				<th data-field="">\
					<div class="th-inner">收据打印张数</div>\
				</th>\
				<th data-field="">\
					<div class="th-inner">收据回收张数</div>\
				</th>\
				<th data-field="">\
					<div class="th-inner">回收差异原因</div>\
				</th>\
			</tr>\
		</thead>\
	</table>';
	$('#J_gapReceiptList').html(gapReceiphtml);
	if(gapReceipdataresult){
		$('#J_gapReceiptList_dataTable').bootstrapTable('destroy');
		$('#J_gapReceiptList_dataTable').bootstrapTable({
			data:gapReceipdataresult.gapReceiptList,
			columns: [ 	
			    ...
			    {field: 'recycleStatus', title: '是否回收', align: 'center',
			    	formatter: function (value, row, index) {
			    		var html = '';
			    		html ='\
			    			<select id="gapselect'+index+'" name="" class="J_chosen form-control">\
			    ...
			    {field: 'recycleCount', title: '收据回收张数', align: 'center',
			    	formatter: function(value, row, index){
	      				var html='';
	           			html='<input type="text" id="gap'+index+'" class="form-control input_return" name="" value="">';
	           			return html;
	           		}
			    },
			    {field: 'differentReason', title: '回收差异原因', align: 'center',
			    	formatter: function(value, row, index){
	      				var html='';
	           			html='<input type="text" id="gapyin'+index+'" class="form-control input_val" name="" value="">';
	           			return html;
	           		}
			    }
			]
...
function yulanfujian($_this){
	commonContainer.modal('查看附件',$('#J_attachmentCon'),function(i){
		...
		success:function(){
			...
			$('#J_businessPreview').text(businessTypeName);
				...
				if(payFs!=4){
					payeehtml='\
						<tr>\
							...
							<td class="add_create" style="text-align: center; padding:8px;"><a type="modify" '+dataobjval+' data-fundCode="'+fundcodenumber+'" class="btn btn-outline btn-success btn-xs btn-payment">修改</a>&nbsp;&nbsp;<a type="del" data-paymentpopmoney="'+paymentpopmoney+'" class="btn btn-outline btn-danger btn-xs">删除</a></td>\
						</tr>';
					...
					$.each(checkrowDataArr,function(i,n){
						...
						tuiPOShtml+='\
							<tr>\
								...
								<td class="add_create" style="text-align: center; padding:8px;"><a type="modify" '+dataobjval+' data-fundCode="'+fundcodenumber+'" data-money="'+idNameMoney+'" class="btn btn-outline btn-success btn-xs btn-payment">修改</a>&nbsp;&nbsp;<a type="del" data-paymentpopmoney="'+idNameMoney+'" class="btn btn-outline btn-danger btn-xs">删除</a></td>\
							</tr>';
					...
					function(index, layero) {
						...
						if(payFs==1 || payFs==2 || payFs==3){
							payeehtml='\
									...
									<td class="add_create" style="text-align: center; padding:8px;">'+paymentUserName+'</td>\
									<td class="add_create" style="text-align: center; padding:8px;"><a type="modify" '+dataobjval+' data-funcode="'+fundcodenumber+'" data-paytype="'+payFs+'" data-isjk="2" class="btn btn-outline btn-success btn-xs btn-payment">修改</a>&nbsp;&nbsp;<a type="del" class="btn btn-outline btn-danger btn-xs">删除</a></td>';
...
/*	var paymentmoeny = $('#J_paymentmoeny').html();
	...
	// 回传差额收据参数
	if(gapReceipdataresult){
		$.each(gapReceipdataresult,function(i,n){
			gapReceiptList.push({
				printCount:n.printCount,
			    receiptId:n.receiptId,
			    receiptNumber:n.receiptNumber,
			    refundAmount:n.amount,
			    returnCount: $('#gap'+i).val(),
			    returnDiffReason:$('#gapyin'+i).val(),
			    isReturn:$('#gapselect'+i).val()
			})
		})
	}
	...
	jsonPostAjax(
		basePath + '/finance/payment/apply/updateApplyInfo',
		{
			...
			"contractNumber":newscontractNumber,
			"fundCode":auditStep[0].fundCode,
			...
			"paymentInfoList":paymentinfolist,                               //回传付款信息
			"gapReceiptList":gapReceiptList                                  //回传差额收据信息
		},
		function(result) {
			layer.msg("操作成功");
