		...
		jsonGetAjax(
			...
			function(result){
				...
				if(newscontractNumber!=''){
					...
					dataresultObj = result.data.paymentInfoList;                     //获取付款新增列表数据
					...
					invoiceListdata = result.data.invoiceList;
					receiptdataresult = result.data.receiptList;
					// 加载有收据的款项列表
					if(receiptdataresult){
						daishouyajin(receiptdataresult);
					}
					// 加载发票数据列表
					invoiceList(invoiceListdata);
					// 付款款项加载数据
					fundName(newscontractId);
					// 付款新增列表数据
		...
		jsonGetAjax(
			...
			function(result){
				...
				if(newscontractNumber!=''){
					...
					$('#J_applyadd').val(result.data.applyInfo.remarks);
					$('#J_annex').show();
					attlistdata=result.data.attList;                                 //获取附件列表信息数据
					dataresultObj = result.data.paymentInfoList;                     //获取付款新增列表数据
					applyIdNumber = result.data.applyInfo.applyId;                   //获取付款申请Id
					paymentfundCode = result.data.applyInfo.fundCode;                //获取付款款项code
					ownerName = result.data.applyInfo.ownerName;
					...
					contractid = result.data.applyInfo.contractId;
					receiptdataresult = result.data.receiptList;
					invoiceListdata = result.data.invoiceList;
					// 加载有收据的款项列表
					if(receiptdataresult){
...
//付款信息款项加载数据
function fundName(newscontractId){
	//初始化付款款项数据
    ...
    jsonGetAjax(
		...
		function(result) {
    	    ...
    	    $('#J_payment').append(options.join(''));
    		$('#J_payment').val(paymentfundCode);
    		$('#J_payment').trigger("chosen:updated");
	});
}
//创建代付押金
function daishouyajin(receiptdata){
	if(paymentfundCode == '5'){	// 待付押金
		//创建代付押金表格表头
		var tabdepositHtml='\
...
function daishouyajin(receiptdata){
	if(paymentfundCode == '8'){
		...
		$('#J_evaluation').show();
		if(receiptdata){
			$('#J_evaluation_dataTable').bootstrapTable('destroy').bootstrapTable({
				singleSelect:false,		//设置单选
				clickToSelect:true,		//点击选中行
				data:result.data,
				columns:[
...
function daishouyajin(receiptdata){
	if(paymentfundCode == '9'){
		...
		if(receiptdata){
			...
			$('#J_registrationtable_dataTable').hide();
		}
	}else{
		$('#J_registration').hide();
	};
...
//返信息费
function invoiceList(invoiceListdataresult){
	if(paymentfundCode == '11'){
		$('#J_Returninformation').show();
		if(invoiceListdataresult){
			var ioversnumber = invoiceListdataresult[0].invoiceCount;
			var selectcode = invoiceListdataresult[0].invoiceDesc;
			var amout = invoiceListdataresult[0].invoiceAmount;
			var Returninformationhtml='';
			$.each(invoiceListdataresult,function(i,n){
				Returninformationhtml='\
					<tr>\
						<td style="width:15%; text-align: center; padding:8px;"><input type="text" placeholder="请输入发票张数" value="'+ioversnumber+'" name="" class="form-control"></td>\
						<td style="width:10%; text-align: center; padding:8px;">\
							<select name="guidstatus" class="J_chosen form-control" id="isIdshenfenz'+i+'">\
								<option value="">请选择</option>\
							</select>\
						</td>\
						<td style="width:15%; text-align: center; padding:8px;"><input type="text" placeholder="请输入发票金额" value="'+amout+'" name="" class="form-control"></td>\
				...
				// 获取发票摘要基础数据
				dimContainer.buildDimChosenSelector($("#isIdshenfenz"+i), "InvoiceSummary", selectcode+'');
				$("select").chosen({
			...
			$('#J_Increasedata').off().on('click',function(){
				var Increasedatahtml='';
					Increasedatahtml='\
					<tr>\
						<td style="width:15%; text-align: center; padding:8px;"><input type="text" placeholder="请输入发票张数" value="" name="" class="form-control"></td>\
						<td style="width:10%; text-align: center; padding:8px;">\
							<select name="guidstatus" class="J_chosen form-control isIdshenfenz" id="isIdshenfenz'+isIdshenfindex+'">\
								<option value="">请选择</option>\
							</select>\
						</td>\
						<td style="width:15%; text-align: center; padding:8px;"><input type="text" placeholder="请输入发票金额" value="" name="" class="form-control"></td>\
			...
			$('#J_Returninformation_dataTable').hide();
		}
	}else{
...
//签约差旅费 
function invoiceList(invoiceListdataresult){
	if(paymentfundCode == '14'){
		$('#J_Signedinformation').show();
		if(invoiceListdataresult){
    		var ioversnumber = invoiceListdataresult[0].invoiceCount;
			var selectcode = invoiceListdataresult[0].invoiceDesc;
			var amout = invoiceListdataresult[0].invoiceAmount;
			var Signedinformation='';
	    	$.each(invoiceListdataresult,function(i,n){
	    		Signedinformation='\
					<tr>\
						<td style="width:15%; text-align: center; padding:8px;"><input type="text" placeholder="请输入发票张数" value="'+ioversnumber+'" name="" class="form-control"></td>\
						<td style="width:10%; text-align: center; padding:8px;">\
							<select name="guidstatus" class="J_chosen form-control" id="isIdz'+i+'">\
								<option value="">请选择</option>\
							</select>\
						</td>\
						<td style="width:15%; text-align: center; padding:8px;"><input type="text" placeholder="请输入发票金额" value="'+amout+'" name="" class="form-control"></td>\
				...
				$('#J_Signedinformation_dataTable tbody').html(Signedinformation);
				// 获取发票摘要基础数据
				dimContainer.buildDimChosenSelector($("#isIdz"+i), "InvoiceSummary", selectcode+'');
				$("select").chosen({
					width : "100%" , no_results_text: "未找到此选项!" 
				});
			});
    		...
    		$('#J_Signedinformation_dataTable').hide();
    	}
	}else{
