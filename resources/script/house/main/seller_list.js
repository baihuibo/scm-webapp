...
$(function(){
	...
	window.onload=function(){
		...
		dimContainer.buildDimChosenSelector($("#houseLevel"), "houseLevel","");//房源级别
		dimContainer.buildDimChosenSelector($("#yesOrNo"), "yesOrNo","1");//房源级别
		/*dimContainer.buildDimChosenSelector($("#proxyStatus"), "proxyStatus","");//房源级别
*/		dimContainer.buildDimChosenSelector($("#customerSource"), "customerSource","");//客户来源
		...
		/*dimContainer.buildDimChosenSelector($("#J_minRommNumber"), "layoutStructure","");//户型
*/		dimContainer.buildDimChosenSelector($(".rommNumber"), "layoutStructure","");//户型
		dimContainer.buildDimCheckBoxHasAll($("#J_houseappraise"), "finalAssessmentIds", "SellerHouseFinalAssessment", "1",'全部');//房源评价
		dimContainer.buildDimChosenSelector($("#J_priceChangeTimes"), "PriceChangeTimes","");//价格变动时间
	...
	jQuery('#J_search').on('click', function(event){
		initListLoad();
		$('#J_dataTable').bootstrapTable('refresh',{ url: basePath + '/house/main/findpagesellers'});
	...
	$('#J_reset_buy').on('click', function(event){
		...
		begindate.max='';
		$("#datetype").val('1');
		$("#customerSourceType").empty();
		$("#customerSourceType_chosen").empty();
	...
	$(document).on("change",'#customerSource',function(){
			...
			if(val==29||val==36||val==45||val==4||val==8){
				var str='<select id="customerSourceType" name="infoSourceId" class="J_chosen form-control m-b" data-placeholder="全部"></select>';
				$(".none").append(str);
			}
			if(val==29){
	...
	function initListLoad(){
		$('#J_dataTable').bootstrapTable({ 
			url:basePath + '/house/main/findpagesellers',
			...
			queryParams : function(params) {
				var o = jQuery('#J_sellerform').serializeObject();
			...
			columns:[
			   {field: '', title: '',align: 'center',
	      	    ...
	      	    {field: 'price', title: '委托价/万',align: 'center',
	           		formatter: function (value, row, index){ 
						var html="";
	      	    ...
	      	    {field: 'unitPrice', title: '单价/元',align: 'right',
	      	    	formatter: function (value, row, index){
	      	    		if(value>0){
	      	    ...
	      	    {field: 'belongUserName', title: '所属人', align: 'center',
	      	    	formatter: function(value ,row, index){
	      	    		var html='';
  	    				if(value!=undefined){
  	    					html='<a class="belongUser" attr="'+row.belongUserId+'">'+row.belongUserName+'</a>'
  	    				}else{
  	    					html='-';
  	    				}	
	      	    		return html;
	      	    ...
	      	    {field: 'registerDate', title: '录入时间', align: 'center',},
	      	 	{field: 'lastFollowDate', title: '最后跟进时间', align: 'center',}
	      	],
		})
	}
