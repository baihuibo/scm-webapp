	...
	$(".J_next,.J_nextadd").click(function(){
		...
		if(nextindex==1){
			if($("#houseNumber").val()==null){
				...
				jsonGetAjax(basePath + '/house/main/checkhousevalidity', {"businesstype":2,"buildinghouseid":$("#houseNumber").val().split(';')[0]}, function(result) {//为了有数据，先注掉用租赁的
					var operationalservicetype= $("#houseNumber option[value='"+$("#houseNumber").val()+"']").attr("operationalservicetype");
					if(operationalservicetype.indexOf("2") == -1 ){//为了有数据，先注掉用租赁的
			...
			if($("#census").val()==1){
				if($("#censusChectoutType").val()==null){
					commonContainer.alert("请填写具体户籍迁出时间！");
					return;
				}
			}
			...
			var taxType='';
			if(val.housePriceInfoVo.taxtype!=undefined){
				for(var i=0;i<val.housePriceInfoVo.taxtype.length;i++){
					taxType+="##"+ val.housePriceInfoVo.taxtype[i];
				}
				val.housePriceInfoVo.taxtype=taxType+"##";
			}else{
				val.housePriceInfoVo.taxtype=taxType;
			}
			val.businesstype=2;
			...
			jsonPostAjax(basePath + '/house/main/inserthouse', val, function(result) {
				...
				window.location.href="../main/buydetail.htm?houseid="+result.data;//兼容火狐刷新*/
				if(isadd==1){
					if(newhouse==1){
						window.location.href="../main/buydetail.htm?houseid="+result.data;//兼容火狐刷新
					}else{
						//跳转虚拟房源详情
						window.location.href="../virtual/detail.htm?type=2&houseid="+result.data;//兼容火狐刷新
					}
				}else{
					if(newhouse==1){
						window.location.href="../main/buydetail.htm?houseid="+result.data+"&from=addCertificates";//兼容火狐刷新
					}else{
						//跳转虚拟房源详情
						window.location.href="../virtual/detail.htm?type=2&houseid="+result.data+"&from=addCertificates";//兼容火狐刷新
					}
				}
...
/*	$("#checkAll-furniture").click(function(){
		if($(this).is(":checked")){
			...
			$("input[name='furniture']").each(function(){
				var str='<div class="label-num"><input type="text" id="num_'+$("input[name='furniture']").attr('id')+'" class="form-control num"></div>'
	...
	$(document).on("click","input[name='furniture']",function(){
		if($(this).is(":checked")){
			var str='<div class="label-num"><input type="text" id="num_'+$(this).attr('id')+'" class="form-control num"></div>'
				$(this).closest(".checkbox").append(str);
		}else{
