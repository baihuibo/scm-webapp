...
(function(window){
	...
	.controller('inquisitionDetailCtrl', ['inquisitionDetailService', 'signUtil',function(inquisitionDetailService, signUtil){
		...
		$ctrl.inquisitionDetail = function () {	
			inquisitionDetailService.getinquisitionDetailquery({'inquId' :inquId}).then(function(result){
				...
				$ctrl.memo = result.data.housesInquBase.comment;
				$ctrl.isAllDelete=0;
        ...
        $ctrl.delAllPicter=function(){
        	$ctrl.detail.housesPicture1=[];
        	$ctrl.detail.housesPicture2=[];
        	$ctrl.memo='';
        	$ctrl.detail.housesInquBase.inquName='';
        	$ctrl.detail.housesInquBase.inquId='';
        	$ctrl.isAllDelete=1;
        }
        $ctrl.inquese=function(){
        	layer.open({
        		title : '请选择实勘人',
        		type : 1,
        		shift : 1,
        		...
        		yes : function(index, layero) {
        			...
        			var obj={};
        			obj.inquId=inquId;
        			obj.admeasureId=admeasureId;
        			inquisitionDetailService.postinquese(obj).then(function(result){
        				if(result.code==1){
        	    			layer.alert(result.data.describe);
        	    			return false;
        	    		}else if(result.code==0){
        	    			layer.msg("操作成功");
        	    			$ctrl.detail.housesInquBase.inquName=$("#J_user").val();
        	    			$ctrl.detail.housesInquBase.inquId=admeasureId;
        	    		}
    				})
        			/*$.ajax({
        ...
        $ctrl.subimit=function(){
        	...
        	commonContainer.showLoading();
	        	...
	        	obj.memo=$ctrl.memo;
	        	obj.inquId=inquId;
	        	obj.isAllDelete=$ctrl.isAllDelete;
	        	obj.inquUserId=$ctrl.detail.housesInquBase.inquId;
	        	...
	        	inquisitionDetailService.postsubmit(obj).then(function(result){
					...
					window.location.href=basePath + "/house/inquisition/inqCheckPage?inquId="+inquId+'&housesId='+housesId;
				})
	...
	.service('inquisitionDetailService' , ['$http' , function($http){
		this.postsubmit= function (data) {
            return $http.post(basePath + '/house/inquisition/inqEdit', data)
            .then(function (response) {
                return response.data;
            });
		};
		this.postinquese= function (data) {
            return $http.post(basePath + '/house/inquisition/admeasureInqu', data)
            .then(function (response) {
                return response.data;
            });
		};
