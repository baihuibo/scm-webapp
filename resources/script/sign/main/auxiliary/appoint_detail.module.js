var res={};
(function(window){
	angular.module('auxiliary_detail' , ['base' , 'sign-common'])
	
	.controller('auxiliaryDetailCtrl' , ['auxiliaryDetailService','signUtil',function(auxiliaryDetailService,signUtil){
		var $ctrl = this;	
		...
		//合同信息
		{
			$ctrl.subsidiarylist=function(){
				auxiliaryDetailService.getSubsidiaryquery({'subsidiaryContractId' :subsidiaryContractId,"subsidiaryContractType":1}).then(function(result){
					if(result.code !== 0){
		...
		//审批流程
		{
			 $ctrl.contractApprovalList=function(){
				 auxiliaryDetailService.getcontractApproval(conId).then(function(result){
						if(result.code !== 0){
							return layer.alert(result.msg);
						}
		...
		//打印历史
		{
			$ctrl.printhistory=function(){
				auxiliaryDetailService.getprinthistory(conId).then(function(result){
					if(result.code !== 0){
						return layer.alert(result.msg);
					}
	...
	.service('auxiliaryDetailService' , ['$http' , function($http){
        ...
        this.getprinthistory=function(conId){
        	 return $http.get(basePath + '/sign/signthecontract/printhistory' , {params : {conId : conId,type:3}}).then(function (response) {
                 return response.data;
             });
        }
	}]);
	 ...
	 // 获取查看客源地址
	this.getContractLink = function (conId) {
         return basePath + '/sign/signthecontract/contractdetail.htm?conId=' + conId;

    };
	
	
	angular.bootstrap(document , ['auxiliary_detail']);
}(window));