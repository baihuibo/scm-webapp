...
$(function(){
	...
	jsonPostAjax(
		...
		function(result) {
			//$('#J_signnumberid').val(result.data.signnum);
			auditbyfinance.init(result.data.signnum, result.data.isPay, result.data.isChargeback, result.data.isRecieve);			
		}
	);
			...
			$('#J_btn_button').delegate(
					...
					function(event) {
						...
						if(result.data.strbusinesstype=='买卖'){ // 判断业务类型 开始分支 走买卖
							...
							if(datavalbutton == 'toJumpFirstNode'){
								var arry = [];
								$('#J_financecheck input[name="financecheck"]:checked').each(function(){
									arry.push($(this).data('value'));
								})
								if(arry.length<1){
									layer.alert('驳回原因必填项不能为空');
									return false;
								}
								var arrstr = arry.toString();
								var contenttext = $('#J_auditrecordcontent').val();
								var arrstrcontent = arrstr + ' ' + contenttext;
								
								jsonPostAjax(basePath+'/workflow/doJob?modelName=BUY_CHARGEBACK&methodName=toJumpFirstNode',{
									taskId:taskId,
									formId:result.data.chargebakcid,
									signnum:signnumber,
									comment:arrstrcontent,
									isChargeback:ischargeback,
									isPay:isPaynum,
									isRecieve:isRecieveType
								},function(){
							...
							if(datavalbutton == 'toJumpFirstNode'){
								...
								jsonPostAjax(basePath+'/workflow/doJob?modelName=RENT_CHARGEBACK&methodName=toJumpFirstNode',{
									taskId:taskId,
									formId:result.data.chargebakcid,
									signnum:signnumber,
									comment:arrstrcontent,
							...
							$.each(rdata.data.enclosureList,function(i,n){
								html+='\
					    			<div class="col-md-3" style="padding-top:12px;text-align: center;" data-filename="" data-filepath="">\
					    				...
					    				<button type="button" data-opt="del" class="btn btn-outline btn-success btn-xs mt-3" onclick="auditbyfinance.download(\''+n.filePath+'\')">下载</button>\
				    				</div>';
							});
