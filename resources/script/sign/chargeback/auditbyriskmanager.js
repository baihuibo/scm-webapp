...
$(function(){
	...
	jsonPostAjax(
		...
		function(result) {
			auditbyriskmanager.init(result.data.signnum, result.data.isPay, result.data.isChargeback, result.data.isRecieve);
		}
	);
	//初始化退单类型数据
	dimContainer.buildDimChosenSelector($("#J_reasonwithdrawal"), "reasonwithdrawal", "");
});

//标签table切换 所展示的数据总方法
var auditbyriskmanager={
	signnumber:$('#J_signnumberid').val(),	//单据编号
	init:function(signnumber, isPaynum, ischargeback, isrecieve){
		var _$this = this;
			...
			$('#J_btn_button').off().delegate(
				...
				function(event) {
					...
					if(result.data.strbusinesstype=='买卖'){ // 判断业务类型 开始分支 走买卖
						var reasoncheck = $('#J_reasonwithdrawal').val();
						if(datavalbutton == 'toPass'){
							if(reasoncheck == '1'){
								reasoncheck = '甲方违约';
							}else if(reasoncheck == '2'){
								reasoncheck = '乙方违约';
							}else if(reasoncheck == '3'){
								reasoncheck = '央产无法办理';
							}else if(reasoncheck == '4'){
								reasoncheck = '政策性房屋无法办理';
							}else if(reasoncheck == '5'){
								reasoncheck = '我司原因';
							}else if(reasoncheck == '6'){
								reasoncheck = '原补充协议的解决事宜发生';
							}else if(reasoncheck == '7'){
								reasoncheck = '国家政策';
							}else if(reasoncheck == '8'){
								reasoncheck = '甲乙双方协商';
							}else if(reasoncheck == '9'){
								reasoncheck = '其他';
							}
							if(reasoncheck ==''){
								layer.alert('退单原因必填项不能为空');
								return false;
							}
							
							var reasoncheckcontent = reasoncheck + ' ' + audittextcontent;
							//查询流程创建的审批人
							...
							$.each(rdata.data.enclosureList,function(i,n){
								html+='\
					    			<div class="col-md-3" style="padding-top:12px;text-align: center;" data-filename="" data-filepath="">\
					    				...
					    				<button type="button" data-opt="del" class="btn btn-outline btn-success btn-xs mt-3" onclick="auditbyriskmanager.download(\''+n.filePath+'\')">下载</button>\
				    				</div>';
							});
