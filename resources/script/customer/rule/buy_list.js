...
//					$('.valRule').text(result.data.rulelist[1].rulevalue);
            ...
            $('#J_ruleListContent').html(content);

        });

    function getButton(v, rulevalue) {
        if (v.rulename === '转介功能' && !buy_cooperation_permission) {
            return '';
        } else if (v.rulename === '带看超期' && !buy_showtimeout_permission) { // 带看超期
            return '';
        }
...
$('#J_ruleList').delegate('#J_checkRule', 'click', function (event) {
    if ($(this).attr('data-type') == '12') { // 转介
        setRule($(this));
    } else if ($(this).attr('data-type') == '13') { // 带看
        setRule($(this));
    }
})
 ...
 * 启用/关闭规则设置
 *
 * @param type：规则设置的类型（12,转介；13,带看）
...
function setRule($this_) {
    ...
    if (curChecked == true) { // 选中状态
        if (type == '13' && (ruleValue == '' || ruleValue == "null")) {
            setGuide(curId, ruleValue); //带看
        }

        // 启用规则设置
        if (type == '12' || (type == '13' && ruleValue != '')) {
            jsonGetAjax(
