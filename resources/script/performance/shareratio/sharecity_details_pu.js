...
$(document).delegate('.J_shreinfo_modify', 'click', function(event){
 ...
 var ruleDetailId =currentEditTr.find("td").eq(0).find("div.perfType").attr("data-id");
	...
	//lowerLimit = tr.eq(5).text().substr(0, tr.eq(5).text().length - 1);
...
//	var tr= $("#J_dataTable").find("tr").eq(1).find("td");
  ...
  /* var totalRow = 0 
	$('#J_dataTable_details tr').each(function() { 
 ...
 $.ajax({
		type : "POST",
		url : basePath + '/perf/setRuleDetail/updateSaveP',
		dataType : "json",
...
//textarae字数限制
 ...
 window.dimContainer1 = {
			...
			buildDimChosenSelector1: function($container, compId, selectedValues) {//selectedValues默认选中值
				...
				var that = this;
			    var options = [];
			    jsonPostAjax(that.getDimReqUrl(), {'compId':compId,'keyId':1}, function(result) {
