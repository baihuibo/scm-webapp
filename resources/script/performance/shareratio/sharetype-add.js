		...
		layer.open({
	    	...
	    	yes: function(index, layero) {
	    		...
	    		if($("#J_AllowAdjust").val()=="1"){
		    		...
		    		if(parseInt($("#J_lowerLimit").val()>$("#J_upperLimit").val())){
		    			commonContainer.alert("分单下限不能大于分单上限 ！");
		    			 return false;
		    		 }
		    		if(parseInt($("#J_lowerLimit").val())==parseInt($("#J_upperLimit").val())){
...
$("#mome").on("input propertychange", function() {  
    var $this = $(this),  
        ...
        count = "";  
    if (_val.length > 260) {  
        $this.val(_val.substring(0, 260));  
    }  
    count = 260 - $this.val().length;  
...
//	}
  ...
  if(curlen >= len) {
	...
	$("#save-add-city1").attr("disabled", "disabled");
	}
 }
