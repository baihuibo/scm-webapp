...
/*详情*/
 $('#J_dataTable_details').bootstrapTable({ 
		url:basePath + '/perf/setRuleDetail/getPerformanceDeail',
		           ...
		           // {field: 'ruleDetailId', title: '序号', align: 'center'},
		      	    {field: 'valueName', title: '业绩分单类型', align: 'center',
		      	    	formatter:function(value, row, index){
			    			return '<div class="perfType" type="'+row.perfType+'" data-id="'+row.ruleDetailId+'" mome="'+row.memo+'" date1="'+row.compName+'">'+value+'</div>'
						}	
		      	    },
...
//$('#J_dataTable_details').bootstrapTable('hideColumn', 'ruleDetailId');
$('#J_dataTable_details').bootstrapTable('refresh',{ url: basePath + '/perf/setRuleDetail/getPerformanceDeail'});
 if(delfalg==1){
  ...
  * 1、新增城市规则模态框
  */
	$(document).delegate("#add-sharetype","click",function(event){
		...
		$("#J_isAllowAdjust").val("");
		$('#J_isAllowAdjust').trigger('chosen:updated');
    /*
     * 增加表单
     * */
	    	...
	    	yes: function(index, layero) {	
	    		...
	    		$("#J_dataTable_1>tbody").find("tr").each(function(){
	    			...
	    			var sharetypeval=$("#J_perfType").find("option:selected").val();
		  			...
		  			var AllowAdjust=$("#J_isAllowAdjust").find("option:selected").text();
		  			var lowerLimit1="-";
		  			var upperLimit1="-";
		  			var movePercent1="-";
		  			...
		  			$('#J_DefaultCalculate').trigger('chosen:updated');
	    		  			var tr='\
	    						   <tr><td data-text='+sharetype+' ><div class="perfType" type='+sharetypeval+'>'+sharetype+'</div></td>\
...
$(document).delegate('.J_shreinfo_modify', 'click', function(event){
   ...
   //selectDefaultCalculateValue(currentEditTr.find("td").eq(1).text(),"#J_DefaultCalculate",0);
	$("#J_perfType").attr("data-id",ruleDetailId)
	...
	selectDefaultCalculateValue(currentEditTr.find("td").eq(4).text(),"#J_isAllowAdjust",0); 
	    if($("#J_isAllowAdjust").val()==1){
	...
	layer.open({
   	...
   	yes: function(index, layero) {
		...
		var sel_value=$("#J_perfType").find("option:selected").text();
			...
			if($("#J_isAllowAdjust").find("option:selected").text()=="请选择"){
		...
		$("#J_dataTable_details").find("tr").eq(currentIndex).find("td").eq(0).attr("data-id",ruleDetailId);
   		...
   		$("#J_dataTable_details").find("tr").eq(currentIndex).find("td").eq(4).text($("#J_isAllowAdjust").find("option:selected").text());
		if($("#J_isAllowAdjust").val()=="1"){
 ...
 //下拉框判断
$("#J_isAllowAdjust").bind("change",function(){ 
	    if($(this).val()==1){
	    	$("#J_lowerLimit").removeAttr('disabled', 'disabled');//移除只读
	    	$("#J_upperLimit").removeAttr('disabled', 'disabled');//移除只读
	    	//$("#J_movePercent").removeAttr('disabled', 'disabled');//移除只读
...
$(document).delegate("#save-add-city","click",function(event){	
    ...
    if(departmentTypeId_val!=0){
		layer.alert('适用公司只能选择公司,不能选择部门');
		return false;
	}
   var ere={};
   ere.cityids = Arr;
